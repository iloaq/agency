...
import { RelatedCasesCarousel } from "@/components/cases/related-cases-carousel";
import { SiteMarketingFooter } from "@/components/site/site-marketing-footer";
import { fetchCaseStudyBySlug, fetchPublishedCaseStudies } from "@/lib/cases/fetch-case-studies";
import {
  asStack,
  asStringArray,
  formatStackLine,
...
export default async function CaseStudyPage({ params }: PageProps) {
  const { slug } = await params;
  const [row, allPublished] = await Promise.all([fetchCaseStudyBySlug(slug), fetchPublishedCaseStudies()]);
  if (!row) notFound();

  const relatedCases = allPublished.filter((r) => r.slug !== row.slug).map(rowToCaseCard);
  ...
  const flowImageUrl = row.architecture_flow_image_url?.trim() ?? "";
    ...
    <main className="isolate min-h-screen overflow-hidden bg-[#F6F3EE] text-[#121212]">
      ...
      <section className="px-5 pb-12 pt-10 sm:px-8 lg:px-10 lg:pb-18 lg:pt-14">
        <div className="grid gap-8 lg:grid-cols-[0.66fr_0.34fr] lg:items-end">
          ...
          <div className="w-full rounded-[30px] border border-[#E6E0D8] bg-white p-6 shadow-[0_16px_46px_rgba(72,57,41,0.055)] lg:max-w-none lg:justify-self-end">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#6B6B6B]">Маршрут проекта</p>
            {flowImageUrl ? (
              <div className="relative mt-3 w-full overflow-hidden rounded-[22px] bg-[#F6F3EE] ring-1 ring-[#E6E0D8]/80">
                {/* eslint-disable-next-line @next/next/no-img-element -- URL из БД, домены не фиксированы; см. https://nextjs.org/docs/messages/no-img-element */}
                <img
                  src={flowImageUrl}
                  alt={`Схема потока: ${row.title}`}
                  className="mx-auto block max-h-[min(52vh,520px)] w-full object-contain object-center"
                  loading="eager"
                  decoding="async"
                />
              </div>
      ...
      <RelatedCasesCarousel cases={relatedCases} />
